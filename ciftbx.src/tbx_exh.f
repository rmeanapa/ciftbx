...
C
C
C                     CIF Tool Box Application 'tbx_exh.f'
C                     ------------------------
C                     Version: Febrary 2024
C
C
...
C     at the start of the tool box source file 'ciftbx.f'.  This
C     version of tbx_ex.h, named tbx_exh.f, is the show the effect
C     if use of the phtml_ logical flag to change from text format
C     output to html output.
C
...
C     dictionary file 'cif_core.dic' has been used [note that other
...
C
C
C....... Because this is a test of internal functions,  common variable file
C        'ciftbx.sys' must be present and each of the ciftbx functions needs
C        a declaration.
C
         include       'ciftbx.sys'
         logical        char_
...
C
C        fudge to deal with gfortran vs. g77
C
         slash = rs(1:1)
C
C
...
C
         if(dict_('cif_core.dic','valid dtype'))  goto 100
         write(6,'(/a/)') ' Requested Core dictionary not present'
C
C        Restore the old clipping action for text fields
C
         clipt_ = .true.
         pclipt_ = .true.
C
...
C
C
C....... Open a new CIF as an html
C
400      phtml_ = .true.
         logdnam_ = .true.
         if(pfile_('testh.htm'))  goto 450
...
C
         f1 = pcmnt_(' ') 
         f1 = pcmnt_(' All of this is in datablock "whoops_a_daisy"')  
