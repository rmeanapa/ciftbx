...
C<\\\\\\\\_ |_____________________________|&&& 21 Nov 2000  &&||          |
C          \|     ||#####|________________|&&&&&&&&&&&&&&&&&&&||__________|
C                  |#####|
C                  |#####|                Version 2.6.4 Release
C                  |#####|
C                 /#######\ 
C                |#########|
C                    ====
...
210        if(char_(ck(i),name)) then
     ...
     *         (' Multiple DDL 1 and 2 related key definitions ')
             icktype = i
             if (newdent(name(1:max(1,long_)),ick)) then
               catkey(ick) = .true.
             else
...
C
         loop_ = .false.
         loopnl = 0
         if(char_('_xml_mapping.token',xmtoken)) then
230        if(char_('_xml_mapping.token_type',xmtyp)) then
             if(char_('_xml_mapping.target',xmtarg)) then
               if (xmnxlat.ge.XMLDEFS) then 
                 call err(' XML translations > XMLDEFS')
               else
                 xmnxlat=xmnxlat+1
                 xmlate(xmnxlat)=xmtarg
               endif
               ...
               if (xmtyp.eq.'category') then
                 ixmtyp = 2
                 nxmc = ndcname+1
                 call hash_store(locase(xmtoken(1:lastnb(xmtoken))),
 ...
 235                 xmindex(ifind) = xmnxlat
           ...
           xmtoken = bname
           if(char_('_xml_mapping.token_type',xmtyp)) then
             if(char_('_xml_mapping.target',xmtarg)) then
               if (xmnxlat.ge.XMLDEFS) then 
                 call err(' XML translations > XMLDEFS')
               else
...
250      if(.not.char_(nt(intype),name)) goto 200
         kdict=ndict+1
251      if (newdent(name(1:long_),ifind)) then
         ...
         if(ifind.le.idstrt) then
           if(kdup)252,253,254
252        call err(' Duplicate name in dictionary '//
...
C
         if(baname.ne.' ') then
           if (newdent(baname(1:lbaname),iafind)) then
             dictag(iafind)    =batag
...
C
         ...
         ilen = lastnb(xname)
         jck = ndict
         call hash_store(locase(xname(1:ilen)),
     *     dicnam,dicchain,
...
c
         ihi = lenn
c
100      if (ihi.gt.16) then
           if (str(ihi-15:ihi).eq.'                ') then
             ihi = ihi-16
             go to 100
           endif
         endif
...
C        matching change in xxd2chr.  Be certain to have at least
...
C
         ...
         include  'ciftbx.sys'
C
         xxd2chr = char(d+ichar(' '))
C
C        The code above may not be portable, especially to non-ascii
C        computer systems.  In that case, comment out the line above
C        and uncomment the following lines.  Be sure to make the
C        matching change in xxc2dig.  Be certain to have at least
C        XXRADIX characters in the search string.
C
...
C
         ...
         mode = 0
         klen = 4
         bstr(1:4) = xxd2chr(0)//xxd2chr(0)
     *     //xxd2chr(0)//xxd2chr(0)
         do ii = 1,ialen
           c = astr(ii:ii)
           if (mode .eq. -2) then
             krep = xxc2dig(bstr(klen-1:klen-1))
             if (c.eq.bstr(klen:klen).and.krep.lt.XXRADIX-1) then
               bstr(klen-1:klen-1) = xxd2chr(krep+1)
             else
               mode = 0
           ...
           if (mode .ge.-1 .and. mode .le.2) then
             ...
             if (c.eq.XXFLAG .and. klen.lt.iblen-1) then
               bstr(klen+1:klen+2) = xxd2chr(1)//c
               mode = -2
               klen = klen+2
             endif
           endif
           if (mode.eq.2) then
             bstr(klen-2:klen-1) = XXFLAG//xxd2chr(3)
             mode = -2
           endif
         enddo
 100     mlen = klen
         do ii = 4,1,-1
           bstr(ii:ii) = xxd2chr(mod(klen,XXRADIX))
           klen = klen/XXRADIX
         enddo
         return
         end
...
C
         ...
         klen = 0
         do ii = 1,4
           klen = klen*XXRADIX+xxc2dig(bstr(ii:ii))
         enddo
         mode = 0
         ipos = 0
         do ii = 5,klen
...
C
         subroutine xxflin(linno,lip,lipag,lipof,kip,ip,mip,mis)
C
         include   'ciftbx.sys'
...
C
     ...
     *                    form='FORMATTED',recl=NUMCPP)
         ...
         if(append_ .and. nrecd.ne.0) then
           call xxflin(nrecd,lip,lp,mp,kip,ip,mip,mis)
           if (lip.eq.0) then
             call err(' Failed to find end of direct access file')
           endif
           kp = 1
           lp = nfblock
           nfblock = nfblock+1
...
160      read(cifdev,'(a)',end=180) buffer
     ...
     *     call warn(' Input line length exceeds line_')
         call xxrle(buffer(1:klen),scrbuf,mlen)
         ...
         mlen = mlen*NUMCIP
         call xxflin(nrecd,lip,lppag,lipof,kip,ip,mip,mis)
...
100      continue
         ...
         mpp=-1
         npp=kpp
         call xxflin(irecd,lip,kpp,mp,kip,ip,mip,mis)
         if (lip.eq.0) then
           buffer = ' '
           go to 130
         endif
...
120      call xxrld(buffer,pagebuf(lpp)(mp:NUMCPP))
130      recn_=irecd
...
C
         subroutine procat(name,type,flag,tflag,mycat,myxcat,
     *     item,xitem,nroot)
C
         ...
         character  locase*(MAXBUF), upcase*(MAXBUF)
         ...
         if (mycat(1:1).ne.' ' .and. name(1:1).eq.'_') then
           if(str1.eq.str2(2:kpl+1) .and. npl .gt. kpl+2 .and.
...
C
         ...
         call procat(name,'numb',flag,tflag,mycat,myxcat,
     *     item,xitem,temp)
         pnumb_=flag
...
C
         if(name(1:1).eq.' ')   goto 110
         call procat(name,'char',flag,tflag,mycat,myxcat,
     *     item,xitem,temp)
         pchar_=flag
100      if(ploopn.ne.0)        call eoloop
         if (xmlout_) then
...
130      if(ploopf.eq.'yes') ploopc=0
         ...
         if(string(1:1).eq.' '.and.ll.gt.1) then
           ...
           call putstr(temp2(1:lastnb(temp2)))
           pchar=-1
           return
         endif
         pchar=-1
...
C
         call procat(name,'    ',flag,tflag,mycat,myxcat,
     *     item,xitem,str)
         ploop_ = flag
...
120      if(ploopn.le.0) then
           ...
           if(xmlout_ .and. plcat.ne.mycat) then
             ...
             npl = lastnb(mycat)
             kpc = pchar
             call putstr('<!--  mixed categories '//plcat(1:kpl)
     *                      //' and '//mycat(1:npl)//' -->')
             pchar = kpc
           endif
         endif
...
C
         ...
         data plhead     /NUMLP1*' '/
         data prefx      /' '/
         data tbxver_    /'CIFtbx version 2.6.4 21 Nov 2000'/
         data lprefx     /0/
         data esdlim_    /19/
         data esddig_    /0/
         data pesddig_   /0/
